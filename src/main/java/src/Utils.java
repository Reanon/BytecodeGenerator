...
public class Utils {
    ...
    public static String getDisasmCode(String binary){
        ...
        try{
            ...
            // 指定 evm 位置
            // disasmCode = runCMDWithTimeout(new String[]{"/usr/local/evm-1.8.14/evm-1.8.14", "disasm", tmp.getAbsolutePath()});
            disasmCode = runCMDWithTimeout(new String[]{"/usr/local/geth-1.8.14/evm", "disasm", tmp.getAbsolutePath()});
        }catch (Exception e){
            e.printStackTrace();
        }finally {
            tmp.delete();
...
//            int lineID = Utils.Hex2Int(tmp[0]);
    ...
    public static void printAnswerCheck(ArrayList<Boolean> groundTruth, BytecodeGenerator bytecodeGenerator){
